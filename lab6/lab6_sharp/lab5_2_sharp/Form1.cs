...
namespace lab5_2_sharp
{
    public partial class Form1: Form
    {
        ...
        private readonly DataBinaryLoader _dataLoader = new DataBinaryLoader();
        ...
        private void BtnCalculate_Click(object sender, EventArgs e)
        {
            ...
            foreach (TabPage tab in validTabs)
            {
                ...
                {
                    var inputs = GetInputValues(tab);
                    _calculationManager.ProcessCalculation(inputs);
                }
                catch (Exception ex)
                {
        ...
        private CalculationInputs GetInputValues(TabPage tab)
        {
            ...
            return new CalculationInputs(X0, Y0, Xk, (int)yCount, StepX, StepY, tab.Text);
        }
        ...
        private void ValidateInputs(CalculationInputs inputs, string tabName)
        {
            // Проверка количества точек по X
            ...
            if(inputs.YCount < 1)
            {
                throw new Exception("Кол-во y не менее 1");
            }
        }
        ...
        private void BtnLoadData_Click(object sender, EventArgs e)
        {
            using(var dialog = new OpenFileDialog())
            {
                ...
                {
                    var data = _dataLoader.ReadFromBinaryFile(dialog.FileName);
                    if (data != null)
                    {
        ...
        private void ShowPointsForTab(TabPage tab)
        {
            ...
            try
            {
                ...
                var (xValues, yValues) = PointGenerator.GeneratePoints(
                    ...
                    inputs.Y0, (int)inputs.YCount, inputs.StepY);
        ...
        private void LoadFileList()
        {
            ...
            try
            {
                ...
                var files = Directory.GetFiles(resultsPath, "G*.rez")
        ...
        private void ListBoxFiles_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Файл должен иметь расширение .rez", "Ошибка",
                              MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            try
            {
                if (radioSubmatrix.Checked)
                {
                    string input = rangeTextBox.Text.Trim();
                    var match = Regex.Match(input, @"\((\d+),(\d+)\)\((\d+),(\d+)\)");

                    if (!match.Success)
                        throw new FormatException("Неверный формат. Используйте (y1,x1)(y2,x2)");

                    int yStart = int.Parse(match.Groups[1].Value);
                    int xStart = int.Parse(match.Groups[2].Value);
                    int yEnd = int.Parse(match.Groups[3].Value);
                    int xEnd = int.Parse(match.Groups[4].Value);

                    //if (yStart < 0 || xStart < 0 || yEnd < 0 || xEnd < 0)
                    //{
                    //    MessageBox.Show($"Координаты должны быть в пределах матрицы.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    //    return;
                    //}

                    if (yStart > yEnd || xStart > xEnd)
                    {
                        MessageBox.Show("Начальная координата не может быть больше конечной.", "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }

                    data = _dataLoader.ReadSubmatrixFromBinaryFile(filePath, yStart, xStart, yEnd, xEnd);
                }
                else if (radioFullMatrix.Checked)
                {
                    data = _dataLoader.ReadFromBinaryFile(filePath);
                }
